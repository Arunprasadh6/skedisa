...
class Email {
    ...
    )
    {
        ...
        $html = $this->CI->load->view('emails/appointment_details', [
            ...
            'appointment_provider' => $provider['first_name'] . ' ' . $provider['MRN'],
            'appointment_start_date' => date('d-m-Y H:i A',strtotime($appointment['start_datetime'])),
            'appointment_end_date' => date('d-m-Y H:i A',strtotime($appointment['end_datetime'])),
            ...
            'customer_name' => $customer['first_name'] . ' ' . $customer['MRN'],
 ...
 public function send_refund_email(EmailAddress $recipient_email,$appid,$orgid){
             $mailer = $this->create_mailer();
        $mailer->From = "Raja@gravitykey.com";
        $mailer->FromName = "Gravitykey.com";
        $mailer->AddAddress($recipient_email->get());
        $mailer->Subject = "Skedis Refund ";
       
        $mailer->Body .= "Dear Customer,<br>";
         $mailer->Body .= "Your appointment with $orgid (#$appid) has been cancelled by you. So, the amount will be refunded through the same mode in which payment was made by you. If refund is made within 24 hours of scheduled appointment, certain amount may be deducted from the actual appointment fee paid.";
          $mailer->Body .="<br>";
          $mailer->Body .="Thanks<br>";
           $mailer->Body .="Team Skedis";
 ...
 public function send_register_email(EmailAddress $recipient_email,$name,$org){
             $mailer = $this->create_mailer();
        $mailer->From = "Raja@gravitykey.com";
        $mailer->FromName = "Gravitykey.com";
        $mailer->AddAddress($recipient_email->get());
        $mailer->Subject = "Skedis Registration-Welcome!";
        $mailer->Body .= "<b>Hey There ".$name."</b>,\n<br>";
        $mailer->Body .= "<br>";
        $mailer->Body .= "Thanks for registering with $org through SKEDIS. You've just signed up for the ease of booking appointments that lets you plan for your arrival with the clinics and other business locations, without having to wait in long queues or make telephone calls!\n<br>";
        $mailer->Body .= "<br>";
         $mailer->Body .= "You can use the same credentials to book appointments at other participating partner locations to avail a wide range of services.";
          $mailer->Body .="<br>";
          $mailer->Body .="Thanks";
           $mailer->Body .="Team Skedis";
