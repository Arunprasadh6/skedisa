...
(function () {
    ...
    CustomersHelper.prototype.bindEventHandlers = function () {
        ...
        //     instance.resetForm();
        //     instan   ce.filter(key);
        // });

        $('#customers').on('keyup', '#filter-customers form', function (event) {
            event.preventDefault();
            var key = $('#filter-customers .key').val();
            $('#filter-customers .selected').removeClass('selected');
            instance.filterLimit = 20;
        ...
        $('#customers').on('click', '#cancel-customer', function () {
            ...
            $('#appointment1').css('display','block');
        });
        ...
        $('#customers').on('click', '#save-customer', function () {
            var customer = {
                ...
                Organization: $('#provider-organization').text(),
...
hr = ("0"+hr).slice(-2);
         var apdate=start.split(' ');
    
         var apdate1=apdate[0].split('/');
        var aphr=apdate[1].split(':');        
        var tme=month+"-"+day+"-"+yr+" "+hr+":"+min+" "+ampm;
        var tme1=apdate1[1]+"-"+apdate1[2]+"-"+apdate1[0]+" "+aphr[0]+":"+aphr[1]+" "+apdate[2];         
        var tme=new Date(Date.parse(tme));
        var tme1=new Date(Date.parse(tme1));

      //  console.log(tme+" "+tme1);
                ...
                'html': [
                    ...
                    $('<a/>', {
                        ...
                        'html': [
                            ...
                            $('<strong/>', {
                                'text': appointment.service.name + ' - ' + appointment.provider.first_name + ' ' + appointment.provider.MRN
                            }),
                            $('<br/>'),
                        ]
                    }),
    ...
    CustomersHelper.prototype.getFilterHtml = function (customer) {
        var name = customer.first_name + ' ' + customer.MRN;

        var info = customer.email;

        info = customer.email ? info + ', ' + customer.phone_number+", "+customer.Create_date : customer.phone_number+", "+customer.Create_date;
