...
(function(exports) {
    ...
    // Variables
    var lastFocusedEventData; // Contains event data for later use.i
    ...
    function bindEventHandlers() {
        ...
        $calendarPage.on('click', '.btn-delay', function() {
            var $dialog;
            // var startDatetime;
            // var endDatetime;
            ...
            $('#delay-phone').val(appointment.customer.phone_number);
            $('#delay-model').modal('show');
            $(this).parents('.popover').popover('dispose');
        });

        $calendarPage.on('click', '.btn-comp', function() {
            var appointment = lastFocusedEventData.data;
            var aid = appointment.id;
            url = GlobalVariables.baseUrl + '/index.php/backend_api/update_status';
            var data = {
            ...
            $.post(url, data)
                .done(function() {
                    $(this).parents('.popover').popover('dispose');
                    Backend.displayNotification("Status Changed");
                });
            $(this).parents('.popover').popover('dispose');
        });

        $calendarPage.on('click', '.btn-noshow', function() {
            var appointment = lastFocusedEventData.data;
            var aid = appointment.id;
            url = GlobalVariables.baseUrl + '/index.php/backend_api/update_status';
            var data = {
        ...
        $('.delay').on('click', function() {
            ...
            url = GlobalVariables.baseUrl + '/index.php/backend_api/ajax_delay_appointment';
            var dphone = $('#delay-phone').val();
            var dhour = $('#dhour').val();
        ...
        $calendarPage.on('click', '.edit-popover', function() {
            ...
            if (lastFocusedEventData.data.workingPlanException) {
                ...
                //console.log(appointment.start_datetime);
    ...
    function get_mintime() {
        var days = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
        var d = new Date();
        var week = days[d.getDay()];

        var token = GlobalVariables.csrfToken;
        var url = GlobalVariables.baseUrl + '/index.php/backend_api/get_min_time';
        $.post(url, { week: week, csrfToken: token }, function(data) {
           // console.log(data);
            $('#calendar').fullCalendar('option', {
                minTime: data
            });
    ...
    function get_maxtime() {
        ...
        var week = days[d.getDay()];
        var token = GlobalVariables.csrfToken;
        var url = GlobalVariables.baseUrl + '/index.php/backend_api/get_max_time';
        $.post(url, { week: week, csrfToken: token }, function(data) {
       //     console.log(data);
            $('#calendar').fullCalendar('option', {
                maxTime: data
    ...
    function getslt() {
        var filteritem = $('#select-filter-item');
        var value;
        filteritem.on('change', function() {
            value = filteritem.find(":selected").val();
            var url = GlobalVariables.baseUrl + '/index.php/backend_api/get_service_duration';
            $.post(url, { providerid: value, csrfToken: GlobalVariables.csrfToken }, function(data) {
                if (data.length != 0) {

                    $('#calendar').fullCalendar('option', {
                        snapDuration: "00:" + data
                    });
    ...
    function calendarEventClick(event, jsEvent) {
        ...
        var tme = yr + "-" + month + "-" + day + " " + hr + ":" + min;
        var appdate = event.start.format('YYYY-MM-DD HH:mm');

        if (tme > appdate) {
            var bted = '';
        } else {
            var bted = 'edit-popover';
        }
        setTimeout(function(){
            if(tme > appdate){            
                $('.delete-popover').css("display","none");
            }else{
               $('.delete-popover').css("display","block");
               
            }
        },1000);
        
        displayDelay = 'btn-delay';
        displayComplete = 'btn-comp';
        displaynoshow = 'btn-noshow';
        if ($(this).hasClass('fc-unavailable') || $parent.hasClass('fc-unavailable') || $altParent.hasClass('fc-unavailable')) {
                ...
                'html': [
                    ...
                    $('<div/>', {
                        ...
                        'html': [
                            ...
                            // $('<button/>', {
                            //     'class': bted + ' btn btn-primary ' + displayEdit,
                            //     'html': [
                            //         $('<i/>', {
                            ...
                            // }),
                            $('<button/>', {
                                'class': ' btn btn-primary mr-2 ' + displayDelay,
                            ...
                            $('<button/>', {
                                'class': ' btn btn-primary mr-2 ' + displayComplete,

                                'html': [
                                    $('<i/>', {
                                        'class': 'fas fa-clipboard-check mr-2'
                                    }),
                                    $('<span/>', {
                                        'text': "Complete"
                                    })
                                ...
                                'class': ' btn btn-primary mr-2 ' + displaynoshow,
            ...
            $html = $('<div/>', {
                'html': [
                    ...
                    $('<span/>', {
                        'text': event.data ? event.data.provider.first_name + ' ' + event.data.provider.MRN : '-'
                    }),
                    $('<br/>'),

                    $('<strong/>', {
                        'class': 'd-inline-block mr-2',
                        'text': EALang.start+"jkjkhjk"
                    }),
                    $('<span/>', {
                        'text': GeneralFunctions.formatDate(event.start.format('YYYY-MM-DD HH:mm:ss'), GlobalVariables.dateFormat, true)
                    }),
            ...
            $html = $('<div/>', {
                'html': [
                    ...
                    $('<span/>', {
                        'text': event.data.provider.first_name + ' ' + event.data.provider.MRN
                    }),
                    $('<br/>'),
                    ...
                    $('<span/>', {
                        ...
                        'text': event.data.customer.first_name + ' ' + event.data.customer.MRN
                    }),
                    $('<br/>'),
    ...
    function calendarEventResize(event, delta, revertFunc) {
        ...
        if (Boolean(Number(event.data.is_unavailable)) === false) {
            ...
            var unavailable = {
                id: event.data.id,
                start_datetime: event.start.format('YYYY-MM-DD HH:mm'),
        ...
        return $.post(url, data)
            .done(function(response) {
                ...
                response.appointments.forEach(function(appointment) {
                    var na = (appointment.customer == null) ? '' : appointment.customer.first_name;
                    var mrn = (appointment.customer == null) ? '' : appointment.customer.MRN;
                    ...
                    switch (calendarView.name) {
                        case 'agendaDay':
                            ...
                            if (sortedWorkingPlan[weekdayName] === null) {
                                ...
                                console.log(calendarView.intervalStart.clone() + "\n" + calendarView.intervalEnd.clone());
    ...
    exports.initialize = function() {
        ...
        GlobalVariables.availableServices[0]['duration'];

        //"00:"+GlobalVariables.availableServices[0]['duration'],
        ...
        $('#calendar').fullCalendar({
            ...
            select: function(start, end) {
                ...
                //      if(check < today)
                //      {
                //  Notiflix.Notify.Warning('Please Select Another Time');
                //  return false;
                //     }
                //      else{
                ...
                // var ul = GlobalVariables.baseUrl + '/index.php/backend_api/get_orderid';
                // var da = {
                //     csrfToken: GlobalVariables.csrfToken
                // };
            ...
            $('<optgroup/>', {
                    ...
                    'html': GlobalVariables.availableProviders.map(function(availableProvider) {
                        ...
                        return $('<option/>', {
                            ...
                            'text': availableProvider.first_name + ' ' + availableProvider.MRN
                        })
                    })
                })
