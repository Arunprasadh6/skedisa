...
(function(exports) {
    ...
    //$("#start-datetime").datepicker("option","minDate",0);
    ...
    function bindEventHandlers() {
        ...
        $('#manage-appointment #save-appointment').on('click', function() {
            ...
            var appointment = {
                ...
                Organization: $dialog.find('#model-org').text(),
                Order_id: $dialog.find('#order').val()
            };
            ...
            var successCallback = function(response) {
                ...
                Notiflix.Notify.Success("Appointment Booking Success");
                setTimeout(() => {                   
                    window.location=GlobalVariables.baseUrl+'/backend/dashboard_admin';
                }, 2000);
            ...
            var ul = GlobalVariables.baseUrl + '/index.php/backend_api/update_orderid';
            var da = {
                csrfToken: GlobalVariables.csrfToken,
                orderid: $dialog.find('#order').val()
            };
            ...
            //yyyy-MM-dd HH:mm:ss'
            var stdat = yr + "-" + month + "-" + day + " " + hr + ":" + min + ":" + sec;
            // console.log(stdat+" "+startDatetime);


            if (startDatetime < stdat && GlobalVariables.editAppointment == null) {
                Notiflix.Notify.Warning("Please Choose Present date & Time");
            } else {
                ...
                if (GlobalVariables.editAppointment != null) {
                    ...
                    //setTimeout(function() { window.location = "/index.php/backend/index"; }, 1000);
        ...
        $('#select-customer').on('click', function() {
            ...
            if (!$list.is(':visible')) {
                ...
                GlobalVariables.customers.forEach(function(customer) {
                    $('<div/>', {
                            'data-id': customer.id,
                            'text': customer.first_name + ', ' + customer.phone_number
                        })
                        .appendTo($list);
                });
            } else {
                ...
                $.post(url, data)
                    .done(function(response) {
                        ...
                        response.forEach(function(customer) {
                            $('<div/>', {
                                    'data-id': customer.id,
                                    'text': customer.first_name + ' ' + customer.MRN
                                })
                                .appendTo($list);
                    ...
                    .fail(function(jqXHR, textStatus, errorThrown) {
                        ...
                        GlobalVariables.customers.forEach(function(customer, index) {
                            if (customer.first_name.toLowerCase().indexOf(key) !== -1 ||
                                customer.MRN.toLowerCase().indexOf(key) !== -1 ||
            ...
            GlobalVariables.availableProviders.forEach(function(provider) {
                provider.services.forEach(function(providerServiceId) {
                    ...
                    if (Number(providerServiceId) === Number(serviceId)) {
                        $('#select-provider')
                            .append(new Option(provider.first_name + ' ' + provider.MRN, provider.id));
                    }
                });
            });
        });
