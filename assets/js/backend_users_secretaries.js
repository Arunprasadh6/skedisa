...
(function () {
    ...
    SecretariesHelper.prototype.bindEventHandlers = function () {
        ...
        $('#secretaries').on('keyup', '#filter-secretaries form', function (event) {
            event.preventDefault();
            var key = $('#filter-secretaries .key').val();
            $('#filter-secretaries .selected').removeClass('selected');
            this.resetForm();
        ...
        $('#secretaries').on('click', '#save-secretary', function () {
            ...
            if( $('#secretary-email').val().length > 0){
            var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
            
            if (reg.test($('#secretary-email').val()) == false) 
            {
        ...
        if($('#secretary-phone-number').val().length <= 9){ 
            ...
            $('#secretary-providers input:checkbox').each(function (index, checkbox) {
                ...
                var ser=$('#secretary-org1').val();
                    if(ser.length==0){
                        $('#secretary-org1').addClass('required');
                        $("#prid").css('color','red');
                    }else{
                         $('#secretary-org1').removeClass('required');
                          $("#prid").css('color','black');
                    }
    ...
    SecretariesHelper.prototype.display = function (secretary) {
        ...
        $('#secretary-last-name').val(secretary.MRN);
        ...
        $('#secretary-org').text(secretary.Organization);
    ...
    SecretariesHelper.prototype.getFilterHtml = function (secretary) {
        var name = secretary.first_name + ' ' + secretary.MRN;

        var info = secretary.email;

        info = secretary.mobile_number ? info + ', ' + secretary.mobile_number : info;

        info = info ? info + ', ' + secretary.phone_number : secretary.phone_number;
