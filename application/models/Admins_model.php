...
class Admins_model extends EA_Model {
    ...
    public function add($admin)
    {
        ...
        if ( ! isset($admin['id']))
        {
            if (isset($admin['phone_number']))
            {
               // $user_id = (isset($admin['id'])) ? $admin['id'] : '';
                if ($this->validate_phone($admin['phone_number'],'') == 1)
                {
                    throw new Exception ('phone_number already exists. Please select a different '
                        . 'phone_number for this record.');
                }
            }
        ...
        else
        {
            ...
            {
                $user_id = (isset($admin['id'])) ? $admin['id'] : '';
                if ($this->validate_phone($admin['phone_number'],$user_id) != 1)
                {
                    throw new Exception ('phone_number already exists or give same phone_number for this record');
                }
            }
    ...
    public function validate_phone($username,$user_id)
    {
    ...
    public function checkmno($mno,$id,$oid)
    {
    $id_roles = $this->get_admin_role_id();    

    $cnt=$this->db->query("select * from ea_users where phone_number='$mno' and id != '$id'  and id_roles='$id_roles'")->num_rows();
       if($cnt > 0){
return "1";
       }else{
return "0";
    ...
    public function exists($admin)
    {
        if ( ! isset($admin['phone_number']))
        {
            throw new Exception('Admin phone_number is not provided: ' . print_r($admin, TRUE));
        }
        ...
        $num_rows = $this->db
            ...
            ->from('users')
            ->join('roles', 'roles.id = users.id_roles', 'inner')
            ->where('users.phone_number', $admin['phone_number'])
            ->where('roles.slug', DB_SLUG_ADMIN)
            ->get()->num_rows();
    ...
    public function find_record_id($admin)
    {
        if ( ! isset($admin['phone_number']))
        {
            throw new Exception('Admin phone_number was not provided: ' . print_r($admin, TRUE));
        }
    ...
    protected function update($admin)
    {
     ...
     //   $this->db->where('phone_number', $admin['phone_number']);
    ...
    public function get_batch($where = NULL, $limit = NULL, $offset = NULL, $order_by = NULL)
    {
        ...
        if($oid==1){
            ...
            $batch=$this->db->get_where('users', ['id_roles' => $role_id,'Organization'=>$oid], $limit, $offset)->result_array();
        }
        
        //$oid=$this->session->userdata('Organization');
        
      //  $cnt = $this->db->get_where('users', ['id_roles' => $role_id,'Organization'=>$oid], $limit, $offset)->num_rows();
    ...
    public function get_appointment_count(){
        $oid=$this->session->userdata('Organization');
        $sql="select COUNT(start_datetime) as total from ea_appointments where Organization='$oid'";
        return $result=$this->db->query($sql)->result_array();         
    }

    public function get_month_cnt(){
        $oid=$this->session->userdata('Organization');
        $mnth=date('m');        
        $sql="SELECT count(*) as count FROM `ea_appointments` WHERE MONTH(start_datetime)='$mnth'   and Organization='$oid'";
        return $result=$this->db->query($sql)->result_array();         
    }
    ...
    public function get_user_count(){
        $oid=$this->session->userdata('Organization');
        $sql="SELECT COUNT(phone_number) as total FROM `ea_users` where Organization='$oid' and id_roles='3'";
        return $result=$this->db->query($sql)->result_array();
    ...
    public function get_service_count(){
        $oid=$this->session->userdata('Organization');
        $sql="SELECT COUNT(name) as total FROM `ea_services` where Organization='$oid'";
        return $result=$this->db->query($sql)->result_array();         
    }

    public function get_cancel_count(){
        $oid=$this->session->userdata('Organization');
        $sql="SELECT count(*) as count FROM `ea_appointments` where Status IN (1,2) AND Organization='$oid'";
        return $result=$this->db->query($sql)->result_array();         
    }
    public function get_daily_count(){
        $oid=$this->session->userdata('Organization');
        $date=date('Y-m-d');
         $sql="SELECT count(*) as count FROM `ea_appointments` where start_datetime like '%$date%' AND Organization='$oid' AND Status='0'";
        return $result=$this->db->query($sql)->result_array();         
    }
    ...
    public function get_chart_count(){
        $oid=$this->session->userdata('Organization');
        $sql="SELECT count(start_datetime) as total, MONTHNAME(STR_TO_DATE(MONTH(start_datetime), '%m')) as month FROM `ea_appointments` where Organization='$oid'  GROUP by MONTH(start_datetime) ";
        return $result=$this->db->query($sql)->result_array();
