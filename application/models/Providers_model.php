...
class Providers_model extends EA_Model {
    ...
    public function add($provider)
    {
        ...
        {
            if (isset($provider['phone_number']))
            {
                // $user_id = (isset($provider['id'])) ? $provider['id'] : '';
               
                if ($this->validate_phone($provider['phone_number'],'') == 1)
                {
        ...
        else
        {
            if (isset($provider['phone_number']))
            {
                ...
                if ($this->validate_phone($provider['phone_number'],$user_id) != 1)
                {
      ...
      public function validate_phone($username,$user_id)
    {
        ...
        $id_roles = $this->get_providers_role_id();
    ...
    {
    $id_roles = $this->get_providers_role_id();    
    $cnt=$this->db->query("select * from ea_users where phone_number='$mno' and id != '$id' and Organization='$oid' and id_roles='$id_roles'")->num_rows();
       if($cnt > 0){
return "1";
       }else{
return "0";
    ...
    public function exists($provider)
    {
        if ( ! isset($provider['phone_number']))
        {
            throw new Exception('Provider phone_number is not provided:' . print_r($provider, TRUE));
        }
        ...
        $num_rows = $this->db
            ...
            ->from('users')
            ->join('roles', 'roles.id = users.id_roles', 'inner')
            ->where('users.phone_number', $provider['phone_number'])
            ->where('roles.slug', DB_SLUG_PROVIDER)
            ->get()->num_rows();
    ...
    public function find_record_id($provider)
    {
        if ( ! isset($provider['phone_number']))
        {
            throw new Exception('Provider phone_number was not provided:' . print_r($provider, TRUE));
        }
    ...
    public function get_batch($where = NULL, $limit = NULL, $offset = NULL, $order_by = NULL)
    {
        ...
        foreach ($batch as &$provider)
        {
            ...
            $provider['secretry_name'] = [];
            foreach ($services as $ser)
            {
               
                $servicenm=$this->db->get_where('services',['id' => $ser['id_services']])->result_array();
                foreach($servicenm as $row){
                    $provider['services_name'][] = $row['name'];
                }
              
                $secnm=$this->db->get_where('secretaries_providers',['id_users_provider' => $provider['id']])->result_array();
                foreach($secnm as $row){
                    
                    $snm=$this->db->get_where('users',['id' => $row['id_users_secretary']])->result_array();
                    foreach($snm as $row){
                        $provider['secretry_name'][] = $row['first_name'];
                    }
    ...
    public function get_available_providers()
    {
        ...
        if($oid==1){
            ...
            ->join('roles', 'roles.id = users.id_roles', 'inner')
            ->where('roles.slug', DB_SLUG_PROVIDER)           
           // ->where('users.organization',$this->session->userdata('Organization'))
            ->order_by('first_name ASC, MRN ASC, email ASC');
        }else{
            $this->db
            ->select('users.*')
            ->from('users')
