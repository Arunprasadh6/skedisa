...
class Appointments_model extends EA_Model {
    ...
    public function add($appointment)
    {
        ...
        if ( ! isset($appointment['id']))
        {
            ...
            $sql="SELECT * FROM `ea_appointments` where (start_datetime BETWEEN '$from' and '$to') OR (end_datetime BETWEEN '$from' and '$to')";    
            $cnt=$this->db->query($sql)->num_rows();
            if($cnt != 0){
                throw new Exception('Please Select Another Slot.');      
            }


            $num_rows = $this->db->get_where('appointments', ['Organization'=>$oid ,'start_datetime'=>$appointment['start_datetime']])->num_rows();
            if($num_rows != 0){
                throw new Exception('Appoinment exist in the database.');      
            }
            else{
        ...
        public function duplicate_add($appointment)
    {
        // Validate the appointment data before doing anything.
       // $this->validate($appointment);
        ...
        // Perform insert() or update() operation.
            ...
            $appointment['id'] = $this->duplicate_insert($appointment);
        // if ( ! isset($appointment['id']))
        // {
        // }
        // else
        // {
        //     $this->duplicate_update($appointment);
        // }
    ...
    protected function insert($appointment)
    {
      ...
      protected function duplicate_insert($appointment)
    {
        $appointment['book_datetime'] = date('Y-m-d H:i:s');
        $appointment['hash'] = $this->generate_hash();

        if ( ! $this->db->insert('duplicate_appointment', $appointment))
        {
            throw new Exception('Could not insert appointment record.');
        }
    ...
    protected function update($appointment)
    {
        ...
        protected function duplicate_update($appointment)
    {
        $this->db->where('id', $appointment['id']);
        if ( ! $this->db->update('duplicate_appointment', $appointment))
        {
            throw new Exception('Could not update appointment record.');
        }
    }
    ...
    public function delete($appointment_id)
    {
      ...
      return  $this->db->query("update ea_appointments set Status='2' where id='$appointment_id'");
       // return $this->db->delete('appointments');
    }
    
    
      public function delete_user($appointment_id,$sta)
    {
        if ( ! is_numeric($appointment_id))
        {
            throw new Exception('Invalid argument type $appointment_id (value:"' . $appointment_id . '")');
       ...
       // $this->db->where('id', $appointment_id);
      return  $this->db->query("update ea_appointments set Status='$sta' where id='$appointment_id'");
       // return $this->db->delete('appointments');
    }
    ...
    public function get_row($appointment_id)
    {
        ...
        return $this->db->get_where('appointments', ['id' => $appointment_id,'Status'=>'0'])->row_array();
    }
    ...
    public function get_value($field_name, $appointment_id)
    {
        ...
        if ($this->db->get_where('appointments', ['id' => $appointment_id,'Status'=>'0'])->num_rows() == 0)
        {
    ...
    public function duplicate_get_value($field_name, $appointment_id)
    {
        if ( ! is_numeric($appointment_id))
        {
            throw new Exception('Invalid argument given, expected integer for the $appointment_id: '
        ...
        if ($this->db->get_where('duplicate_appointment', ['id' => $appointment_id,'Status'=>'0'])->num_rows() == 0)
        {
            throw new Exception('The record with the provided ID does not exist in the database: '
                . $appointment_id);
        }

        $row_data = $this->db->get_where('duplicate_appointment', ['id' => $appointment_id])->row_array();
    ...
    public function get_batch($where = NULL, $limit = NULL, $offset = NULL, $order_by = NULL, $aggregates = FALSE)
    {
        ...
        if($oid==1 OR $oid==8){
            ...
            $this->db->where('Status',0);
            $this->db->where('Organization',$this->session->userdata('Organization'));
        }
    ...
    public function get_batch_app($where = NULL, $limit = NULL, $offset = NULL, $order_by = NULL, $aggregates = FALSE)
    {
    ...
    public function get_mrndata($no){
         
         
         $cnt=$this->db->get_where('users', ['MRN' => $no])->num_rows();
         if($cnt==0){
            echo "not";
         }else{
             return $this->db->get_where('users', ['MRN' => $no])->row_array();
         }
    }


    public function get_country($oid){
        $sql="select Country from ea_orgnization where organization_id='$oid'";
        return  $this->db->query($sql)->result_array();
    }
    
    
    public function get_orderid(){
        $sql="SELECT Orderid FROM `Order_table` ";
        return $this->db->query($sql)->result_array();
    }
        ...
        $query = $this->db->get_where('settings', ['name' => "Company_count",'Organization'=>$oid]);
        $setting = $query->row();
        
       // $date=date('Y-n-j',strtotime("+".$setting->value." days"));
