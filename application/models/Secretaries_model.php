...
class Secretaries_model extends EA_Model {
    ...
    public function add($secretary)
    {
        ...
        {   
            ...
            if (isset($secretary['phone_number']))
            {
               // $user_id = (isset($secretary['id'])) ? $secretary['id'] : '';
                if ($this->validate_phone($secretary['phone_number'],'') == 1)
                {
        ...
        else
        {
            ...
            if (isset($secretary['phone_number']))
            {
                ...
                if ($this->validate_phone($secretary['phone_number'],$user_id) != 1)
                {
...
return "0";
    ...
    public function exists($secretary)
    {
        if ( ! isset($secretary['phone_number']))
        {
            throw new Exception('Secretary phone_number is not provided: ' . print_r($secretary, TRUE));
        }
        ...
        $num_rows = $this->db
            ...
            ->from('users')
            ->join('roles', 'roles.id = users.id_roles', 'inner')
            ->where('users.phone_number', $secretary['phone_number'])
            ->where('roles.slug', DB_SLUG_SECRETARY)
            ->get()->num_rows();
    ...
    public function find_record_id($secretary)
    {
        if ( ! isset($secretary['phone_number']))
        {
            throw new Exception('Secretary phone_number was not provided: ' . print_r($secretary, TRUE));
        }
    ...
    protected function update($secretary)
    {
     ...
     //    $this->db->where('phone_number', $secretary['phone_number']);
    ...
    public function get_batch($where = NULL, $limit = NULL, $offset = NULL, $order_by = NULL)
    {
        ...
        if($oid==1){
            ...
            $batch = $this->db->get_where('users', ['id_roles' => $role_id,'Organization'=>$this->session->userdata('Organization')], $limit, $offset)->result_array();
        }
        ...
        foreach ($batch as &$secretary)
        {
            ...
            $secretary['providers'] = [];
            $secretary['providers_name'] = [];
            foreach ($secretary_providers as $secretary_provider)
            {
                $secretary['providers'][] = $secretary_provider['id_users_provider'];
                $snm=$this->db->get_where('users',['id' => $secretary_provider['id_users_provider']])->result_array();
                foreach($snm as $row){
                    $secretary['providers_name'][] = $row['first_name'];
                }
            }
