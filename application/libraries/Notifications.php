...
class Notifications {
  ...
  public function send_register($mail,$name,$org){
       try
        {
            
            $ma=array("email"=>$mail);
            $email = new EmailClient($this->CI, $this->CI->config->config);
            $email->send_register_email(new Email($ma['email']),$name,$org);
        }
        catch (Exception $exception)
        {
            log_message('error', $exception->getMessage());
...
public function send_refund($mail,$appid,$orgid){
       try
        {
            
            $ma=array("email"=>$mail);
            $email = new EmailClient($this->CI, $this->CI->config->config);
            $email->send_refund_email(new Email($ma['email']),$appid,$orgid);
        }
        catch (Exception $exception)
        {
            log_message('error', $exception->getMessage());
...
public function send_customers($mail,$name,$oid,$subject){
    try{
        $ma=array("email"=>$mail);
        $email = new EmailClient($this->CI, $this->CI->config->config);
        $email->send_customer_email(new Email($ma['email']),$name,$oid,$subject);
