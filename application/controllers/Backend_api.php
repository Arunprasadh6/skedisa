...
class Backend_api extends EA_Controller {
    ...
    public function get_service_duration(){
        $id=$this->input->post('providerid');
        $result=$this->services_model->get_duration($id);
        echo $result;
    }
    
    public function get_min_time(){
        $week=$this->input->post('week');
        $data=$this->services_model->min_time($week);
    echo $data;
    }
    
     public function get_max_time(){
        $week=$this->input->post('week');
        $data=$this->services_model->max_time($week);
        echo $data;
    }
    ...
    public function update_status(){
        ...
        $cnt=$this->db->query("select id,hash,Organization from ea_appointments where id='$appid'")->num_rows();
        if($cnt > 0){
            $this->db->query("update ea_appointments set Status='$status' where id='$appid'");
        }else{
    ...
    public function appointment_report(){
        $status=$this->input->post('status');
       $da=$this->customers_model->get_appointment($status);
        echo json_encode($da);
    }


    public function Store_coupon(){
       $this->settings_model->add_coupon($this->input->post());      
    }

    public function get_coupon(){
        $oid=$this->session->userdata('Organization');
        $sql="select * from ea_coupon where organization='$oid'";
    ...
    public function ajax_save_appointment()
    {
        try
        {
            ...
            $orgd=$this->db->query("SELECT organization_name as org  FROM `ea_orgnization` WHERE organization_id='".$customer['Organization']."'")->result_array();
            $ogan=$orgd[0]['org'];
            $apdate=date('d-m-Y h:i A',strtotime($appointment['start_datetime']));

            $this->synchronization->sync_appointment_saved($appointment, $service, $provider, $customer, $settings, $manage_mode);
            $msg='Dear Customer, your appointment with '.$ogan.' is confirmed at '.$apdate.'  - SKEDIS';
            $this->sms($customer['phone_number'],$msg); 
    ...
    public function ajax_delete_appointment()
    {
        try
        {
            ...
            $appid=$this->db->query("select * from ea_appointments where id='".$this->input->post('appointment_id')."'")->result_array();

            $payment=$this->db->query("select Order_id,TXNid,Amount,TXNDate,Payment_mode,Status,Bank_name from ea_Payment where Order_id='".$appid[0]['Order_id']."'")->result_array();
        ...
        //    @$response_data=$this->paytm_refund_model->refund_customer($payment[0]['Order_id'],$payment[0]['TXNid'],$refundamount);
            ...
            $this->appointments_model->delete_user($this->input->post('appointment_id'),$sta);
            ...
            try
            {
                ...
                foreach ($secretaries as $secretary)
                {
                    ...
                    $email->send_delete_appointment($appointment, $provider,
                        ...
                        $this->sms($customer['phone_number'],$msg);    
                        $this->notifications->send_refund($customer['email'],$appid[0]['Order_id'],$ogan);
                }
            }
            catch (Exception $exception)
            {
     ...
     */
    public function ajax_delay_appointment()
    {
        try
        {
            $dphone=$this->input->post('dphone');
            $dhour=$this->input->post('dhour');
            $reason=$this->input->post('reason');
            $msg='Your appointment will be delayed by '.$dhour.' hour(s) at the clinic due to '.$reason.'. Apologies for the inconvenience. Please reschedule - SKEDIS';
            $this->sms($dphone,$msg);    
    ...
    public function ajax_filter_admins()
    {
        try
        {
            ...
            $where =
                '(first_name LIKE "%' . $key . '%" OR MRN LIKE "%' . $key . '%" ' .
              //  'OR email LIKE "%' . $key . '%" OR mobile_number LIKE "%' . $key . '%" ' .
                ...
                'OR zip_code LIKE "%' . $key . '%" OR notes LIKE "%' . $key . '% " OR Organization LIKE "%' . $key . '%  ")';
    ...
    public function upload_provider_pic(){
      ...
      $this->upload->initialize($config);
             ...
             //update images
              ...
              $filename = $filedata['file_name']; 
               $sql="Update ea_users set Images='$filename' where id='$pid'";
              $this->db->query($sql);         
    ...
    //  print_r($data);
       ...
       $pro=$data['day']['provider'];
      
     $dat=$this->customers_model->get_day_report($day,$pro);
    ...
    $oid=$this->session->userdata('Organization');
    $html='';
    $data=$this->db->query("select * from ea_appointments where start_datetime like  '%$date%' and Organization='$oid' AND Status='0' order by start_datetime asc")->result_array();
     
   //echo $data[0]['status'];
  
  
  if(empty($data)){
    echo "<p style='margin-left:19%;'>No Appointments Available</p>";
  }else{
    ...
    if($status==1 || $status==2 || $status==3 || $status==4){
        ...
        for($i=0;$i<count($data);$i++){
            $result=$this->appointments_model->get_batch(['id' => $data[$i]['id']],null,null,null,1);
           
            foreach($result as $row){
                   $html.="
                   <div class='col-md-3'>
                       <div id='app-tym'>
                           <p>".date('h:i:s',strtotime($row['start_datetime']))."</p>
                           <p>".date('h:i:s',strtotime($row['end_datetime']))."</p>
                       </div>
                    </div>
                    <div class='col-md-9' style='padding:0px'>
                        <div class='tdy-scdule'>
                            <span>".$row['provider']['first_name']."<span><br>
                            <span id='dept'>".$row['service']['name']."</span><br>
                            <span id='usr-mbl'>".$row['customer']['first_name']." (".$row['customer']['phone_number'].")</span>
                                    <div class='dropdown float-end' style='position:
                                        ...
                                        <div
                                            class='dropdown-menu dropdown-menu-end' id='tooltip-container1'>
                                            <!-- item   -->
                                            <a style='color:#000000d1;margin-left: 10%;' href='javascript:void(0);' class='me-3'
                                            data-bs-container='#tooltip-container1'  onclick=editAppointment('".$row['hash']."') data-bs-toggle='tooltip'
                                            data-bs-placement='top' title='Edit' ><i
                                                class='mdi mdi-pencil-outline font-size-18'></i></a>&nbsp;
                                                <a style='color:#000000d1;' onclick='delete_appointment(".$row['id'].",1)' href='javascript:void(0);' 
   ...
   // echo json_encode($result);
    ...
    public function week_wise(){
       ...
       $sdate=$data['week']['startdate'];
       $edate=$data['week']['enddate'];
       $pro=$data['week']['provider'];
      
     $da=$this->customers_model->get_week_report($sdate,$edate,$pro);
     echo json_encode($da);
    }
    ...
    public function month_wise(){
       ...
       $sdate=$data['month']['startdate'];
       $pro=$data['month']['provider'];
      
     $da=$this->customers_model->get_monthly_report($sdate,$pro);
     echo json_encode($da);
    }
    
    public function get_orderid(){
        $data=$this->settings_model->get_order();
        echo $data[0]['Orderid'];
    }
    ...
    public function get_subscription(){
        ...
        $this->paytm_refund_model->subscription();
    }

    public function apply_collpse(){
       $uid=$this->session->userdata('user_id');
       $sql="select Classname,status from ea_users where id='$uid'";
       $data=$this->db->query($sql)->result_array()[0];
       $status=$data['status'];
       if($status==1){
           $this->db->query("update ea_users set status='0' where id='$uid'");
       echo "0";
       }else{
        $this->db->query("update ea_users set status='1' where id='$uid'");
        echo $data['Classname'];
       }
    ...
    public function get_user_data(){
        ...
        $oid=$this->session->userdata('Organization');
        $cnt=$this->db->query("select * from ea_users where phone_number='$mno' AND id_roles='3' AND Organization='$oid'")->num_rows();
        if($cnt > 0){
        $data=$this->db->query("select * from ea_users where phone_number='$mno' AND id_roles='3' AND Organization='$oid'")->result_array();
        echo json_encode($data);
        }else{
        echo "0";
